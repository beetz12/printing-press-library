...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".digg-ai-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DIGG_AI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DIGG_AI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "digg-ai-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.digg-ai-pp-cli/feedback.jsonl.
When ` + "`DIGG_AI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DIGG_AI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  digg-ai-pp-cli feedback list
  digg-ai-pp-cli feedback list --limit 5
  digg-ai-pp-cli feedback list --json`,
