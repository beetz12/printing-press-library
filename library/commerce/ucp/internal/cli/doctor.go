...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ucp-pp-cli doctor
  ucp-pp-cli doctor --json
  ucp-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export API_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ucp-pp-cli", suggestion)
						} else {
