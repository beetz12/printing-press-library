...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ucp-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UCP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UCP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ucp-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ucp-pp-cli/feedback.jsonl.
When ` + "`UCP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UCP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ucp-pp-cli feedback list
  ucp-pp-cli feedback list --limit 5
  ucp-pp-cli feedback list --json`,
