...
```bash
# Run in another terminal — pure-Go reference merchant, no external deps.
ucp-pp-cli mock serve --port 8080

# Probe the mock manifest end-to-end.
ucp-pp-cli check 127.0.0.1:8080 --json

# Search the mock merchant's catalog.
ucp-pp-cli search "coffee" --merchant 127.0.0.1:8080 --limit 5 --json

# Add a line item; creates a new cart and prints its ID.
ucp-pp-cli cart add --merchant 127.0.0.1:8080 --sku coffee_001 --title "Coffee" --price 1500 --qty 2

# List local carts.
ucp-pp-cli cart list

# Produce a checkout draft (JSON) ready for an AP2 CLI to authorize.
ucp-pp-cli checkout prep --cart $(ucp-pp-cli cart list --json | jq -r '.[0].id')
...
This v0.1 ships the search → cart → checkout-prep flow only. The following features from the absorb manifest are deferred to v0.2; track in the GitHub backlog.

- **MCP transport client** — only REST-transport merchants are supported for `search`/`cart`/`checkout prep`. See [Real-Merchant Compatibility](#real-merchant-compatibility) for the full breakdown.
...
The bundled `ucp mock serve` provides a pure-Go reference merchant for end-to-end testing of the REST flow without external dependencies.
