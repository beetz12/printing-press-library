...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ap2-pp-cli",
		Short: `Go CLI for the AP2 (Agent Payments Protocol) — sign and complete AP2 mandates emitted by ucp-pp-cli.`,
		Long: `ap2-pp-cli — A Go CLI for the AP2 (Agent Payments Protocol).

What v0.1 ships:
  • keys generate/list/export    Manage ECDSA-P256 agent keys
  • mandate sign/verify          Sign AP2 FinalizationEnvelopes (from ucp-pp-cli)
  • payment authorize/status     POST signed payment_mandate to merchant complete_checkout (sandbox default)

Pairs with ucp-pp-cli: ucp-pp-cli emits the FinalizationEnvelope, ap2-pp-cli signs + authorizes it.
Agent mode: add --agent to any command for JSON output + non-interactive mode.
Quickstart: ap2-pp-cli keys generate && ap2-pp-cli mandate sign --envelope envelope.json`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ap2-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ap2-pp-cli profile list')")
	...
	rootCmd.AddCommand(newUserKeysCmd(flags))
