...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ap2-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AP2_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AP2_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/payments/ap2/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ap2-pp-cli/feedback.jsonl.
When ` + "`AP2_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AP2_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ap2-pp-cli feedback list
  ap2-pp-cli feedback list --limit 5
  ap2-pp-cli feedback list --json`,
